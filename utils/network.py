...
def build_mlp(
    ...
    with tf.variable_scope(scope):
        ...
        for i in range(n_layers):
            h_scope = "h" + str(i)
            n = tf.contrib.layers.fully_connected(n, num_outputs=size, activation_fn=tf.nn.relu, reuse=tf.AUTO_REUSE, scope=h_scope, trainable=trainable)
        out = tf.contrib.layers.fully_connected(n, num_outputs=output_size, activation_fn=output_activation, reuse=tf.AUTO_REUSE, scope="out", trainable=trainable)
